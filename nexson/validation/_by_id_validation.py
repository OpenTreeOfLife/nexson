#!/usr/bin/env python
from .helper import _NEXEL, error_return
from .schema import check_raw_dict
from .err_generator import (gen_MissingCrucialContentWarning,
...
class ByIdHBFValidationAdaptor(NexsonValidationAdaptor):
    ...
    def _post_key_check_validate_otus_obj(self, og_nex_id, otus_group, vc):
        ...
        if not isinstance(otu_obj, dict):
            self._error_event(_NEXEL.OTUS,
                              ...
                              key_list=['otuById'])
            return error_return('no "otuById" in otus group')
        ...
        try:
            ...
            if not_dict_otu:
                ...
                return error_return('otu is wrong type')
    ...
    def _post_key_check_validate_tree_group(self, tg_nex_id, tree_group_obj, vc):
        ...
        if (not isinstance(tree_by_id, dict)) \
            ...
            return error_return('no "treeById" in trees group')
        ...
        if otus_el not in self._otu_group_by_id:
            self._error_event(_NEXEL.TREES,
                              ...
                              obj_nex_id=tg_nex_id,
                              key_list=[otus_el])
            return error_return('no "@otus" in trees group')
    ...
    def _post_key_check_validate_tree(self,
        ...
        if (not node_by_id) or (not isinstance(node_by_id, dict)):
            self._error_event(_NEXEL.TREE,
                              ...
                              key_list=['nodeById', ])
            return error_return('no "nodeById" in tree')
        if (not edge_by_source) or (not isinstance(edge_by_source, dict)):
            ...
            return error_return('no "edgeBySourceId" in tree')
        if not is_str_type(root_node_id):
            ...
            return error_return('no "^ot:rootNodeId" in tree')
        edge_dict = {}
        edge_by_target = {}
        ...
        if node_set != reachable_nodes:
            ...
            if unreachable:
                ...
                return error_return('unreachable node in tree tree')
            if not_in_node_by_id:
                ...
                return error_return('referenced node id not in "nodeById" in tree')
        if bad_node_ref:
            bad_node_ref.sort()
            self._error_event(_NEXEL.TREE,
                              obj=tree_obj,
            ...
            return error_return('referenced parent node not in "nodeById" in tree')
        if missing_target:
            missing_target.sort()
            self._error_event(_NEXEL.TREE,
                              obj=tree_obj,
            ...
            return error_return('no "@target" in edge')
        if repeated_target:
            ...
            return error_return('same node used as "@target" for different edges')
        if repeated_edge_id:
            ...
            return error_return('edge "@id" repeated')
        ...
        if len(with_at_root_prop) > 1:
            ...
            return error_return('multiple "@root" nodes')
        if len(with_at_root_prop) == 0:
            self._error_event(_NEXEL.TREE,
                              ...
                              anc=vc.anc_list,
                              obj_nex_id=tree_nex_id)
            return error_return('No node labelled as "@root"')
        if root_node_id not in with_at_root_prop:
            ...
            return error_return('root node not labelled as root')
            ...
            if otuid in otuid2leaf:
                ...
                try:
                    self._error_event(_NEXEL.LEAF_NODE,
                                      ...
                                      obj_nex_id=nd_id,
                                      key_list=[otuid])
                    return error_return('Repeated "@otu" id')
                finally:
                    vc.pop_context()
    ...
    def _post_key_check_validate_nexml_obj(self, nex_obj, obj_nex_id, vc):
        ...
        if not isinstance(otus, dict) or (otus_order_list and (not otus)):
            self._error_event(_NEXEL.NEXML,
                              ...
                              key_list=['otusById'])
            return error_return('Missing "otusById"')
        if (not isinstance(otus_order_list, list)) or ((not otus_order_list) and otus):
            ...
            return error_return('Missing "^ot:otusElementOrder"')
        otus_group_list = []
        missing_ogid = []
        not_dict_og = []
        ...
        if missing_ogid:
            ...
            return error_return('Missing "@id" for otus group')
        ...
        try:
            if not_dict_og:
                ...
                return error_return('Otus objects of the wrong type of object')
        ...
        if (not isinstance(trees, dict)) or ((not trees) and trees_order_list):
            self._error_event(_NEXEL.NEXML,
                              ...
                              obj_nex_id=obj_nex_id,
                              key_list=['treesById'])
            return error_return('Missing "treesById"')
        if (not isinstance(trees_order_list, list)) or ((not trees_order_list) and trees):
            ...
            return error_return('Missing "^ot:treesElementOrder"')
        trees_group_list = []
        missing_tgid = []
        not_dict_tg = []
        ...
        if missing_tgid:
            ...
            return error_return('Missing trees group id')
        ...
        try:
            if not_dict_tg:
                ...
                return error_return('Trees element of the wrong type')
